...
SpawnLocalVehicle = function(data)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		TriggerEvent('notification', 'Veuillez retirer le véhicule de la route', 1)
...
SpawnVehicle = function(data,recuperar)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		TriggerEvent('notification', 'Veuillez retirer le véhicule de la route', 2)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                TriggerEvent('notification', 'Votre véhicule est déjà dans les rues de la ville', 2)
    ...
    if recuperar then
        TriggerEvent('notification', 'Vous avez payé 200$ pour récupérer votre véhicule!', 1)
    ...
    if DoesEntityExist(vehicle) then
		...
		esx.TriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
                ...
                CloseMenu()
                TriggerEvent('notification', 'Ce véhicule ne vous appartient pas', 2)
			end
...
SaveInGarage = function(garage)
    ...
    TriggerEvent('notification', 'Vous avez enregistré votre véhicule dans le garage '..garage, 3)
    Citizen.Wait(500)
...
DrawScriptMarker = function(markerData)
    DrawMarker(20, markerData["pos"],0,0,0,0,0,0,0.701,1.0001,0.3001,222, 50, 50, 0.05,0, 0,0,0,0,0,0)
		esx.ShowHelpNotification("~g~E ~w~Accepte ~g~Flèches ~w~Bouge toi ~g~Retour arrière ~w~Exit")
end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return TriggerEvent('notification', 'Ce modèle n\'existe pas dans le jeu, envoyez un rapport!', 2)
    end
